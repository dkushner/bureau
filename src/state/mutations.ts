...
export const mutations: MutationTree<BureauState> = {
  change(state, { formName, fieldName, value }: ChangePayload) {
    ...
    const dirty =
      !resultForm.pristine || isEqual(getIn(resultForm.values, fieldName), getIn(resultForm.initials, fieldName))
    ...
    const sanity = fieldNames.reduce((result: Record<string, any>, name: string): any => {
      const fieldDirty = isEqual(getIn(formState.values, name), getIn(formState.initials, name))
      const recentDirty = isEqual(getIn(formState.values, name), getIn(formState.lastSubmitted, name)) || undefined
  ...
  initialize(state, { formName, values, keepDirty }: InitializePayload) {
    ...
    Object.keys(fields).forEach(key => {
      ...
      if (keepDirty) {
        const pristine = isEqual(get(form.values, key), get(form.initials, key))
  ...
  registerField(state, { formName, fieldName }: RegisterFieldPayload) {
    ...
    if (!fieldState) {
      const initial = form.initials ? getIn(form.initials, fieldName) : undefined
  ...
  unregisterField(state, { formName, fieldName, destroy }: UnregisterFieldPayload) {
    ...
    const resultFields = { ...omit(fields, fieldName) }
